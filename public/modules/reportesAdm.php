...
// Obtener todos los cursos disponibles (sin especificar la cédula)
$sql_cursos = "SELECT c.id_curso, c.nombre_curso FROM cursos c";
...
// Asegúrate de que siempre sea un array, aunque no haya estudiantes en riesgo
$estudiantes_riesgo = $resultado_riesgo->fetch_all(MYSQLI_ASSOC) ?: [];
$total_estudiantes_riesgo = count($estudiantes_riesgo); // Calcular el total de estudiantes en riesgo
...
$stmt_regulares->bind_param("ii", $umbral_bajo, $umbral_alto);
...
// Obtener total de clases dictadas por todos los profesores (sin cédula específica)
$sql_clases_dictadas = "
...
// Promedio de asistencia (sin especificar cédula)
